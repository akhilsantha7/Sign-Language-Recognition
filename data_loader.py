...
class GermanDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        all_openpose_files = sorted(os.listdir(root))
        ...
        # for index, openpose in all_openpose_files:
        # openpose = "01April_2010_Thursday_heute-6694.npy"
        # pdb.set_trace()
        # caption = all_captions[index]
        openpose = all_openpose_files[index]
        image = np.load(os.path.join(root, all_openpose_files[index]))
        image = np.reshape(image, (300, 25, 2)) # #frames, body keypoints, 2 (25 points x 2 (x,y))
...
def collate_fn(data):
    ...
    # lengths = lengths.cpu()
    return images, targets, file_name, lengths
